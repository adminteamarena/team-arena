-- Debug and fix username issue in join/leave messages
-- Add better fallback logic to get username from auth.users
...
BEGIN
    ...
    -- Get joiner name with better fallback logic
    SELECT COALESCE(
        NULLIF(p.full_name, ''), 
        NULLIF(p.username, ''), 
...
BEGIN
    ...
    -- Get leaver name with better fallback logic
    SELECT COALESCE(
        NULLIF(p.full_name, ''), 
        NULLIF(p.username, ''), 
        NULLIF(u.raw_user_meta_data->>'full_name', ''),
